    ...
    return (
        <header className={`h-16 border-b flex items-center justify-between px-3 md:px-8 sticky top-0 z-140 transition-colors duration-300 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-100'}`}>
            <div className="flex items-center gap-6 flex-1 min-w-0">
                {!hideSidebarToggle && (
                    <button
                        ...
                        className={`p-2 rounded-lg border transition-all ${isDark ? 'border-slate-800 text-tertiary hover:bg-slate-800' : 'border-gray-200 text-secondary hover:bg-gray-50'} hover:text-primary hover:border-primary/30`}
                    >
                        <PanelLeft size={20} />
                    </button>
                )}
                <AnimatePresence mode="wait">
                    {(!showSearch || window.innerWidth >= 768) ? (
                        <motion.h1
                            ...
                            className={`text-xl font-bold shrink-0 ${isDark ? 'text-white' : 'text-main'}`}
                        >
                            {getPageTitle()}
                        </motion.h1>
                    ) : null}
            ...
            <div className="flex items-center gap-6">
                <button className="relative p-1 text-tertiary hover:text-primary transition-colors">
                    <Bell size={24} />
