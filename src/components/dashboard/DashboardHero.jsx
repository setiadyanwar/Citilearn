            ...
            <div className="lg:col-span-6 bg-white dark:bg-slate-900 rounded-[28px] p-5 md:p-8 border border-gray-100 dark:border-slate-800 relative flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
                ...
                <div className="absolute inset-0 pointer-events-none select-none overflow-hidden rounded-[28px]">
                    <img
                        ...
                        className="absolute top-0 right-0 h-[30%] md:h-[50%] w-auto object-contain translate-x-4 -translate-y-2 md:-translate-y-2 opacity-95 dark:opacity-40 transition-all duration-700 hover:scale-105"
                    />
                </div>
                ...
                <div className="relative z-10 space-y-4">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            <span className="text-[#059669] dark:text-emerald-400 font-bold text-base md:text-lg">
                                Welcome Back
                            </span>
                    ...
                    {/* Filters Moved Inside Card at Bottom */}
                    {/* Filters: Category and Status side by side with auto gap */}
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-2">
                        ...
                        <div className="flex bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-800 rounded-xl p-1 overflow-x-auto no-scrollbar max-w-full">
                            {['All', 'Aviation', 'Safety', 'Technical', 'Leadership'].map(filter => (
                                <button
                                    key={filter}
                                    ...
                                    className={`text-[11px] md:text-xs font-bold px-3 md:px-4 py-1.5 md:py-2 rounded-lg transition-all whitespace-nowrap ${selectedCategory === filter
                                        ...
                                        : 'text-gray-500 dark:text-slate-400 hover:text-citilink-dark dark:hover:text-white'}`}
                                >
                                    {filter}
                                </button>
                            ))}
                        ...
                        {/* Status Filter Inside Card (Dropdown restored) */}
                        <div className="relative z-[50]" ref={statusDropdownRef}>
                            ...
                            {isStatusDropdownOpen && (
                                <div className="absolute top-full mt-2 left-0 sm:left-auto sm:right-0 w-48 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl shadow-xl z-50 overflow-hidden py-1 animate-in fade-in slide-in-from-top-2 duration-200">
                                    {['All', 'Not Started', 'On Progress', 'Done'].map(status => (
                                        <button
                                            key={status}
                                            onClick={() => {
            ...
            <div className="lg:col-span-6 flex flex-col gap-4">
                ...
                <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-5 md:p-7 rounded-[28px] shadow-[0_4px_20px_rgba(0,0,0,0.01)] flex flex-col sm:flex-row items-center justify-between gap-6 h-full">
                        ...
                        <div className="relative w-14 h-14 md:w-16 md:h-16 flex items-center justify-center shrink-0">
                            <svg className="w-14 h-14 md:w-16 md:h-16 transform -rotate-90" viewBox="0 0 100 100">
                                ...
                                <circle
                                    ...
                                    strokeDashoffset={263.89 * (1 - 0.6)}
                                    className="text-primary"
                                    strokeLinecap="round"
                                />
                            </svg>
                            <span className="absolute inset-0 flex items-center justify-center text-sm md:text-base font-black text-primary">60%</span>
                        </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-center sm:justify-end">
                        {/* Enrolled Badge */}
                        <div className="bg-[#059669] text-white p-2.5 md:p-3 px-4 md:px-5 rounded-xl text-center min-w-[80px] md:min-w-[90px] flex-1 sm:flex-none">
