    ...
    return (
        <section className="space-y-6">
            <div className="flex items-center justify-between pb-2">
                <h2 className="text-xl md:text-2xl font-black text-citilink-dark dark:text-white">Course Curriculum</h2>
                    ...
                    <div key={module.id} className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm transition-all">
                        <div className="px-10 py-5 flex items-center justify-between bg-white dark:bg-slate-900 border-b border-gray-50 dark:border-slate-800">
                            <div className="flex items-center gap-5">
                                ...
                                <h3 className="font-bold text-base md:text-lg text-citilink-dark dark:text-white">{module.title}</h3>
                            </div>
                            ...
                            {module.lessons.map((lesson, lIdx) => {
                                ...
                                return (
                                    <div key={lesson.id} className="px-10 py-5 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors cursor-pointer group/lesson">
                                        <div className="flex items-center gap-5">
                                            <div className="text-gray-300 dark:text-citilink-dark group-hover/lesson:text-primary transition-colors shrink-0">
                                                {lesson.type === 'video' ? <Play size={18} /> : <FileText size={18} />}
                                            </div>
                                            <span className="text-sm md:text-base font-medium text-gray-500 dark:text-slate-400 group-hover/lesson:text-citilink-dark dark:group-hover:text-white transition-colors">{lesson.title}</span>
                                        </div>
                                        {isCompleted ? (
                                            ...
                                            <div className="text-gray-200 dark:text-citilink-dark">
                                                <CheckCircle2 size={18} />
                                            </div>
                                        )}
                                    </div>
