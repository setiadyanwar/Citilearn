                            ...
                            {module.lessons.map((lesson, lIdx) => {
                                ...
                                return (
                                    <div key={lesson.id} className="px-10 py-5 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors cursor-pointer group/lesson">
                                        <div className="flex items-center gap-5">
                                            <div className="text-tertiary dark:text-white group-hover/lesson:text-primary transition-colors shrink-0">
                                                {lesson.type === 'video' ? <Play size={18} /> : <FileText size={18} />}
                                            </div>
                                            <span className="text-sm md:text-base font-medium text-secondary dark:text-slate-400 group-hover/lesson:text-main dark:group-hover:text-white transition-colors">{lesson.title}</span>
                                        </div>
                                        {isCompleted ? (
